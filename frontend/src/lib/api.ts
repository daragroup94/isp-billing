...
function getApiUrl() {
  // If in browser, detect based on hostname
  if (typeof window !== 'undefined') {
    if (window.location.hostname === 'app.access.daragroup.cloud') {
      return 'https://api.access.daragroup.cloud';
    }
  }
...
export const dashboardApi = {
  ...
  getRevenueChart: async (months: number = 12) => {
    const response = await api.get('/dashboard/revenue-chart', { params: { months } });    return response.data;
  },
